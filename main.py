...
def fetch_messages():
    ...
    for block in blocks:
        ...
        reply_to = None
        reply_link = block.select_one("a.tgme_widget_message_reply")
        if reply_link and reply_link.get("href"):
            try:
                reply_to = int(reply_link["href"].rstrip("/").split("/")[-1])
            except (ValueError, IndexError):
                reply_to = None

        messages.append({"id": msg_id, "text": text, "ts": ts, "reply_to": reply_to})
